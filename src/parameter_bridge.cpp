...
int main(int argc, char * argv[])
{
    ...
    topics.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (size_t i = 0; i < static_cast<size_t>(topics.size()); ++i) {
      ...
      bool hasqos = false;
      rclcpp::QoS qos_settings(10); // Set default qos which is overriden
        ...
        if(direction == "1to2") {
          ...
          try {
            if(hasqos){
              ...
              RCLCPP_INFO(ros2_node->get_logger(), "create 1 to 2 bridge for topic %s with non-default qos", topic_name.c_str());
            } else {
              ros1_bridge::Bridge1to2Handles handles = ros1_bridge::create_bridge_from_1_to_2(
                ros1_node, ros2_node,
                "", topic_name, queue_size,
          ...
          try {
            if(hasqos) {
              ...
              RCLCPP_INFO(ros2_node->get_logger(), "create 2 to 1 bridge for topic %s with non-default qos", topic_name.c_str());
            } else {
              ros1_bridge::Bridge2to1Handles handles = ros1_bridge::create_bridge_from_2_to_1(
                ros2_node, ros1_node,
                type_name, topic_name, queue_size,
